document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btn-crear-ticket').addEventListener('click', () => {
        ...
        Store.addTicket({ titulo, categoria, prioridad, descripcion, rol: 'limpieza', autor: session.username });
    ...
    function renderMisTickets() {
        const tickets = Store.getTickets().filter(t => t.rol === 'limpieza');
    ...
    function renderMiHorario() {
        ...
        const miEmpleado = employees.find(e => e.nombre.toLowerCase().includes(session.username.toLowerCase()) || e.rol === 'limpieza');
        ...
        HORAS.forEach(hora => {
            ...
            DIAS.forEach(dia => {
                ...
                if (sch) { totalHoras += 4; td.innerHTML = `<div class="bg-purple text-white p-2 rounded small"><i class="fa-solid fa-lock me-1"></i>${hora}</div>`; }
    ...
    function renderResumen() {
        const schedules  = Store.getSchedules();
        const employees  = Store.getEmployees();
        const miEmpleado = employees.find(e => e.rol === 'limpieza');
