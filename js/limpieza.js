/**
 * limpieza.js — Panel de Personal de Limpieza (solo lectura de sus turnos)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderMiHorario() {
        ...
        const HORAS = ['06:00','08:00','10:00','14:00','18:00','20:00'];

        const schedules  = Store.getSchedules();
        const employees  = Store.getEmployees();

        const miEmpleado = employees.find(e =>
            e.nombre.toLowerCase().includes(session.username.toLowerCase()) || e.rol === 'limpieza'
        );
        ...
        HORAS.forEach(hora => {
            ...
            DIAS.forEach(dia => {
                ...
                if (sch) {
                    totalHoras += 4;
                    const color = sch.tipo === 'fijo' ? 'bg-purple' : 'bg-info';
        ...
        const dias = [...new Set(misTurnos.map(s => s.dia))];
        el('res-turnos', misTurnos.length);
